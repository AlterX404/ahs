...
let paypalHostedDirectContainer = null;
...
function resetPayPalCheckout(mode = selectedDuration) {
  ...
  paypalHostedDirectContainer = null;
...
function setLifetimeHostedButtonEnabled(enabled) {
  const target = paypalHostedDirectContainer || paypalHostedIframe;
  if (!target) return;
...
async function renderPremiumLifetimeHostedButton() {
  if (
    selectedDuration !== "lifetime" ||
    tierSlug !== "premium" ||
    !paypalSubscriptionButton ||
  ...
  const hostedButtonId = "HGZSXXKFJEK4N";
  ...
  paypalHostedDirectContainer = container;
  paypalSubscriptionButton.appendChild(container);
  setLifetimeHostedButtonEnabled(Boolean(termsCheckbox?.checked));

  try {
    // This is the official PayPal Hosted Buttons integration supplied for
    // Premium Lifetime. The matching hosted-buttons SDK is loaded once in
    // premium/index.html, while the monthly SDK uses its own namespace.
    if (!window.paypal || typeof window.paypal.HostedButtons !== "function") {
      throw new Error("PayPal Hosted Buttons SDK is not available.");
    }

    const result = window.paypal.HostedButtons({
      hostedButtonId: "HGZSXXKFJEK4N"
    }).render("#paypal-container-HGZSXXKFJEK4N");

    await Promise.resolve(result);

    if (selectedDuration !== "lifetime" || tierSlug !== "premium") return;

    paypalHostedButtonRendered = true;
    setLifetimeHostedButtonEnabled(Boolean(termsCheckbox?.checked));
  } catch (error) {
    console.error("PayPal Premium Lifetime Hosted Button error:", error);
    paypalHostedDirectContainer = null;
    showPayPalStatus(
      "PayPal lifetime checkout could not load. Please refresh and try again.",
      true,
      true
    );
  } finally {
...
function renderPayPalHostedButton() {
  if (tierSlug === "premium") {
    void renderPremiumLifetimeHostedButton();
    return;
  }
